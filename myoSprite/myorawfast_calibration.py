...
class MyoRaw(object):
    ...
    def connect(self):
        ...
        def handle_data(p):
            ...
            self.magnitude = np.sqrt(np.mean(emg_temp ** 2, axis=0))

            if len(self.label_list) < 1200:
                self.label = self.gesture_list[0]
            elif len(self.label_list) < 1800:
                self.label = self.gesture_list[1]
            elif len(self.label_list) < 2400:
                self.label = self.gesture_list[2]
            elif len(self.label_list) < 3000:
    ...
    def set_leds(self, logo, line):
        self.write_attr(0x19,
                        pack('8B', 6, 6, *(logo + line)))  # logo and line should be lists of 3 elements => color code
...
class MyoMain():
    ...
    def emg_handler(self, emg, *args):  # *args: if more argument needs to be passed to the function
        ...
        self.window.status = self.mr.status
        self.window.length = self.mr.length
